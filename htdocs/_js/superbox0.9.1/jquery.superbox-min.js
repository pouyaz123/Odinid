(function(i){var l,t,r,q,a,p,h,o,j,w,pd,bovr,$innerbox,b={boxId:"superbox",boxClasses:"",overlayOpacity:0.5,boxWidth:"600",boxHeight:"400",loadTxt:"Loading...",closeTxt:"Close",prevTxt:"Previous",nextTxt:"Next",beforeShow:function(){}},x={},m=false,s=i([]);
i.superbox=function(){w=i.extend({},b,i.superbox.settings);if(i.browser.msie&&i.browser.version<7){s=s.add("select")
}if(!$('#superbox').length)n();z()};function z(){i("a[rel^=superbox],area[rel^=superbox]").each(function(){
var D=i(this),F=D.attr("rel"),B=F.match(/^superbox\[([^#\.\]]+)/)[1];
var E = F.split('[');E = E[1] ? (E[1].match(/([#\.][^#\.\]]+)/g) || []) : [];
var C=w.boxId,A=w.boxClasses;
...
B.click(function(D){D.preventDefault();k();y(function(){var E=false;if(C&&C[1]){E=C[1].split("x")
}A=i.extend({},A,{boxWidth:E[0]||A.boxWidth,boxHeight:E[1]||A.boxHeight});q.width(A.boxWidth+"px");
$innerbox.height(A.boxHeight+"px");
if(!pd)pd={loc:window.location.href, ttl:DocumentTitle?DocumentTitle:document.title, lngSub:$("#divLangSubNav").html(), cbarURL:typeof(objCbarURL)!='undefined'?objCbarURL.url:null, copyr:$('#tdCopyright').html()}
PostBack.Post(B, $innerbox, C[0], [], 'INSERT', true);
e({boxClasses:"ajax "+A.boxClasses,boxId:A.boxId});u()})})}});function f(A){return A._relSettings.match(/([^\[\]]+)/g)
}function g(A,B){}function c(C,D){h.show();m=true;var A=C.get(0)._superboxGroupKey+1,B=A-2;if(x[D][A]){o.removeClass("disabled").unbind("click").bind("click",function(){x[D][A].click()
...
}function d(){i(document).unbind("keydown");p.hide();h.hide();t.hide().css({position:"fixed",top:0});TinyMCE_destroyer(t);
$innerbox.empty()}function v(A){
if(pd){_t.PushState(pd.loc);_t.DocTitle(pd.ttl);if(pd.lngSub)$("#divLangSubNav").html(pd.lngSub);if(pd.cbarURL)objCbarURL.url=pd.cbarURL;if(pd.copyr)$('#tdCopyright').html(pd.copyr);pd=null}
d();l.fadeOut(300,function(){s.show()});m=false}function y(B){var A=function(){if(i.browser.msie&&i.browser.version<7){t.css({position:"absolute",top:"50%"})
}s.hide();p.show();B()};if(m){l.css("opacity",w.overlayOpacity).show();A()}else{l.css("opacity",0).show().fadeTo(300,w.overlayOpacity,A)
}}function k(){TinyMCE_destroyer(t);t.show();$innerbox.empty();q.css({position:"absolute",top:"40px"})
}function u(A,B){p.hide();i(document).unbind("keydown").bind("keydown",function(C){
if(C.keyCode==27){v()}if(C.keyCode==39&&o.is(":visible")){o.click()}if(C.keyCode==37&&j.is(":visible")){j.click()}
...
t=i('<div id="superbox-wrapper"></div>').appendTo("form:first").hide();var t2=i('<div id="superbox-table"></div>').appendTo(t);r=i('<div id="superbox-container"></div>').appendTo(t2);
...
p=i('<p class="loading">'+w.loadTxt+"</p>").appendTo(r).hide();a.click(function(){v()
