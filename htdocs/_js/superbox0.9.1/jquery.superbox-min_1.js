...
if(!pd)pd={loc:window.location.href, ttl:DocumentTitle?DocumentTitle:document.title, lngSub:$("#divLangSubNav").html(), cbarURL:typeof(objCbarURL)!='undefined'?objCbarURL.url:null}
...
if(pd){_t.PushState(pd.loc);_t.DocTitle(pd.ttl);if(pd.lngSub)$("#divLangSubNav").html(pd.lngSub);if(pd.cbarURL)objCbarURL.url=pd.cbarURL;pd=null}
...
}}function k(){TinyMCE_destroyer(t);t.show();$innerbox.empty();q.css({position:"absolute",top:"-99999px"})
