...
RETURNS INT(10)	--check your _universityschool_info datatable to ensure about identical type
BEGIN
-- 	DECLARE $schoolID INT(10);
-- 	DECLARE $tagID CHAR(17);
	SET $schoolID = (SELECT `ID` FROM `_universityschool_info` WHERE `ID` = $schoolID
		OR (
			`Title` = $title
			AND (
				`Domain` <=> $domain
...
-- ESCAPE char is same as what we have in T\DB::LikeEscapeChar has been used in T\DB::EscapeLikeWildCards
	IF ISNULL($schoolID) THEN
		INSERT INTO `_universityschool_info`(`Title`, `Domain`, `URL`) VALUES($title, $domain, $url);

		SET $schoolID = LAST_INSERT_ID();
-- 		SET $tagID = schools_getCreatedSchoolTagID($schoolID);
-- 
-- 		UPDATE `_universityschool_info` SET `TagID` = $tagID WHERE ID = $schoolID;
...
-- RETURNS CHAR(17)	--check your _universityschool_info datatable to ensure about identical type
