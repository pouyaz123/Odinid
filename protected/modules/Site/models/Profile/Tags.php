 ...
 */
class Tags extends \Base\FormModel {
//
...
//	}
	...
	static function AC_GetSuggestions($term) {
		if ($term) {
			$Items = T\DB::GetField("SELECT GROUP_CONCAT(`Tag` ORDER BY `IsOfficial` DESC, `Tag` SEPARATOR ',')"
							. " FROM `_simpletags`"
							. " WHERE `Tag` LIKE CONCAT(:term, '%') ESCAPE '" . T\DB::LikeEscapeChar . "'"
