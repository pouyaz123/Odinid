...
_t.RunScriptAfterLoad(['tagit/tag-it.min', 'MyJuiAutoComplete/MyAutoComplete', 'tagit/ac_urlFactor'], function() {
	tagit_ac_companies(
		'#'+<?=$Model->PostName?>+'_txtCompanyTitle', '#'+<?=$Model->PostName?>+'_hdnCompanyID', '',
		'<?= Tools\HTTP::URL_InsertAjaxKW("AutoComplete_Prj_txtCompanyTitle") ?>', <?= Settings::GetInstance()->MaxProjectCompanies ?>)
})
</script>
					<? /* 	hdnID;
					  #
