...
class ResendActivation extends \CAction {
	...
	public function run() {
		$this->controller->pageTitle = \t2::SitePageTitle(\t2::site_site('Resend Activation Link'));
		...
		if ($btn && $Model->ResendActivationLink()) {
			\Output::Render($this->controller, '../messages/success', array('msg' => \t2::site_site('Activation link has been sent')));
		} else
