...
class RecoveryCode extends \CAction {
	...
	public function run($code = null) {
		$this->controller->pageTitle = \t2::SitePageTitle('tr_user', 'Reset password');
		...
		if ($btn && $Model->Recover()) {
			\Output::Render($this->controller, '../messages/success', array('msg' => \t2::Site_User('Your new password has been set successfully.')));
		} else
