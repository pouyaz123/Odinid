...
namespace Site\controllers\Tutorials;
...
class EditCategories extends \CAction {
	...
	public function run() {
		$this->controller->pageTitle = \t2::SitePageTitle(\t2::site_site('Tutorial Categories'));

		$Model = new \Site\models\Tutorials\Categories('Add');
		...
		if ($btnAdd || $btnSaveEdit) {
			$Model->attributes = \GPCS::POST('TutCat');
			$Model->Save();
		} elseif ($btnEdit)
			$Model->SetForm();
		elseif ($btnDelete)
		...
		\Output::Render($this->controller
				, ($btnEdit ?
						'/tutorials/cats_addedit' : '/tutorials/cats')
				, array(
			'Model' => $Model,
	 ...
	 */
	private function DataGrid(\Site\controllers\ProfileController $ctrl, \Site\models\Tutorials\Categories $Model) {
